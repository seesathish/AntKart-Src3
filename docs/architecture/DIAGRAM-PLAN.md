...
> **Documentation only — no diagrams are drawn yet.** This file records the plan for the cloud-native diagram set. The diagrams themselves are produced during the upcoming deep-dive sessions; this document is the contract they follow (visual language, tooling, the list, and source of truth).
...
Status is **Planned** for all 18 — none drawn yet. "Source of truth in this repo" is where the diagram (once produced) is authored/kept.
...
| 06 | Terragrunt unit dependency graph (all 19 units) | In what order do the IaC units apply, and what depends on what? | Mermaid | `docs/architecture/DIAGRAM-PLAN.md` → the guide it lands in (`infrastructure/README.md`) | Planned |
| 07 | Network & traffic path — DNS → TLS → **APIM** → ingress → gateway | How does a request physically reach a service, and where is TLS terminated? | Mermaid | `docs/guides/aks-guide.md` | Planned |
| 08 | Identity & trust chain — Terraform SP → GitHub OIDC → workload identity → RBAC | How does trust flow from provisioning to runtime, secret-lessly? | Mermaid | `docs/guides/identity-concepts.md` | Planned |
| 09 | Security posture & trust boundaries — secret-less chain, public vs ClusterIP, **KI-002** | What is exposed vs internal, and where do the known gaps sit? | Mermaid | `docs/test/SECURITY_TESTS.md` / `docs/KNOWN_ISSUES.md` | Planned |
| 10 | **APIM** edge — policy chain, JWT validation, two-gateway model (ADR-020) | What does the managed edge do before traffic reaches the cluster ingress? | Mermaid | `docs/adr/ADR-020-api-management-managed-edge-gateway.md` | Planned |
...
| 12 | Workload identity token flow | How does a pod get an Entra token with no stored secret? | Mermaid (sequence) | `docs/guides/identity-concepts.md` | Planned |
| 13 | Helm chart & values precedence | How does one generic chart become six services, and which values win? | Mermaid | `deploy/helm/README.md` | Planned |
| 14 | CI pipeline | What runs on a pull request, and what gates the merge? | Mermaid | `docs/guides/devops-cicd-guide.md` | Planned |
| 15 | CD pipeline | What happens on merge — build, push, tag-bump — and with what identity? | Mermaid | `docs/guides/devops-cicd-guide.md` | Planned |
| 16 | GitOps reconciliation loop | How does a Git change become a running pod via Argo CD? | Mermaid | `docs/guides/gitops-guide.md` | Planned |
| 17 | Environment promotion — dev vs QA | How does a change move from dev to QA, and what differs between them? | Mermaid | `docs/ROADMAP.md` (until an environments guide exists) | Planned |
| 18 | Observability pipeline | How do logs, metrics, and traces flow to their sinks and dashboards? | Mermaid | `docs/design/OBSERVABILITY.md` | Planned |

## 5. Status

All **18** diagrams are **Planned**. None are drawn. This file is updated (per-row status → produced, and the superseded old asset removed) as each diagram is delivered in the deep-dive sessions.
