...
- [ADR-025 — Observability Architecture: Serilog for Logs, OpenTelemetry for Traces and Metrics](ADR-025-observability-architecture.md) — Split the three signals by tool: Serilog JSON to the console collected into Log Analytics `ContainerLog`, OpenTelemetry traces to Application Insights, Prometheus `/metrics` exposition; OTel log export left off to avoid double ingest, OTel pinned to 1.13.x on net9, managed Grafana and sampling deferred.
