...
- **The Architect's Playbook** — a study reference covering **70 concepts across eight sections** (Platform, Infrastructure as code, Azure services, Kubernetes, Security and identity, Observability, GitOps, DevOps), each written to the full per-concept template. Every concept carries a status tag — 🟡 To start, 🔵 In progress, 🟢 Proven — and reaches 🟢 only once it can be explained aloud, without notes, with its gotchas recalled and its code located. The document is written; **proving the concepts is ongoing work**, not a document still to be authored — [Architect's Playbook](ARCHITECT-PLAYBOOK.md).
...
- **Make the provisioning runbook followable from an empty subscription.** The runbook today is a *clone* procedure — it assumes an existing environment and this author's specific Azure tenant, so a developer arriving with only a subscription cannot follow it. Closing this is what turns the repository from a reference into something another developer can build from. Four sub-items, in priority order: (1) a "Start here" section answering the four first-time-reader questions; (2) a Phase -1 covering subscription prerequisites, tooling, provider registration, and creating the Terraform service principal across all four permission planes; (3) creating the state backend that Phase 0 assumes exists; (4) parameterising globally-unique names by project or organisation, not only by `$ENV`. **Items 1 and 2 alone deliver most of the value**, and the result is proven only by writing it up and validating it — a stranger following it end to end from an empty subscription to a running platform — see the runbook's Appendix C for the detail: [Environment Provisioning Runbook](guides/environment-provisioning-runbook.md).
