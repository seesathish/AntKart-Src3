...
- Shared cross-cutting library (DDD base types, auth, messaging, resilience, middleware) — [Building Blocks](../AK.BuildingBlocks/BUILDING_BLOCKS.md)
...
- Messaging on Azure Service Bus with MassTransit, over Entra token auth against IaC-owned topology — [ADR-015](adr/ADR-015-messaging-migration-to-service-bus.md) · [ADR-007](adr/ADR-007-masstransit-over-raw-rabbitmq.md) · [Messaging concepts](guides/messaging-concepts.md)
...
- **Helm packaging of the six services** — charts for Deployments, Services, probes wired to `/health/live` and `/health/ready`, the workload-identity ServiceAccount annotations and pod labels, and per-environment values sourced from [Container Configuration](guides/container-configuration.md). See the [AKS Guide](guides/aks-guide.md#still-to-come).

## Planned — near term

- **Ingress and TLS termination** — an ingress controller fronting the gateway, with TLS termination for external traffic.
...
- **Hardened container base image** — a chiseled/distroless .NET base image published to ACR and consumed by all service images, reducing the attack surface and centralising runtime patching in one place.
