...
### 4. Kubernetes Platform · _Status: in progress (cluster and workload identity complete; service deployment pending)_

The services are containerized and run on a managed Kubernetes (AKS) cluster, with ingress, autoscaling, health management, and workload identity. **Complete:** containerization (multi-stage, non-root Dockerfiles on port 8080, a repository-root `.dockerignore`, images published to the Azure Container Registry); the **AKS cluster** `aks-antkart-dev` (Azure CNI Overlay, Azure RBAC, OIDC issuer, OMS agent, kubelet AcrPull); and **per-service workload identity** (a federated user-assigned identity per service, least-privilege roles, verified reading Key Vault from a pod with no stored secret). **Still to come:** deploying the services onto the cluster via Helm, ingress/TLS, and GitOps delivery.

- **Build guide:** [AKS Guide](docs/guides/aks-guide.md) — container strategy, the provisioned cluster, operator access, workload identity, and troubleshooting (Helm/ingress/GitOps sections are placeholders until delivered)
