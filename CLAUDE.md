...
- **Seed data:** seeding is out-of-band via `AK.Tools.DiscountSeedLoader` (no boot-time seeding — startup only applies migrations). It seeds coupons whose `ProductId` is a **real Cosmos product `_id`** (so `GetDiscount(product_id)` resolves and `discountPrice` is computed) — category rules (Kids 15%, Men/Shirts 10%) plus a deterministic ~20% spread. The loader is **authoritative**: it clears all existing coupons first, then seeds, so every run converges to the same clean set. (The old in-process `DiscountSeeder`, which wrote SKU-keyed sample rows that never matched a Cosmos id, was removed.)
- **Tests:** 49 passing (domain, commands, queries, validators, DTO mapping)
